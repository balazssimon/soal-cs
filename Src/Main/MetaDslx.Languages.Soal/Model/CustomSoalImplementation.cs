...
namespace MetaDslx.Languages.Soal.Symbols
{
    ...
    public static class SoalExtensions
    {
        ...
        public static Annotation AddAnnotation(this AnnotatedElement annotatedElement, string annotationName)
        {
            Annotation result = annotatedElement.GetAnnotation(annotationName);
            if (result == null)
            {
                var f = new SoalModelFactory(annotatedElement.MModel);
                result = f.Annotation();
        ...
        public static void SetAnnotationPropertyValue(this AnnotatedElement annotatedElement, string annotationName, string propertyName, object value)
        {
            var annot = annotatedElement.Annotations.FirstOrDefault(a => a.Name == annotationName);
            if (annot == null)
            {
        ...
        public static void SetPropertyValue(this Annotation annotation, string propertyName, object value)
        {
            var prop = annotation.Properties.FirstOrDefault(p => p.Name == propertyName);
            if (prop == null)
            {
                var f = new SoalModelFactory(annotation.MModel);
        ...
        private static void CheckArrayType(SoalType type, IList<Annotation> annotations, HashSet<string> arrayNames, List<ArrayType> arrayTypes)
        {
            ...
            if (type is ArrayType)
            {
                ...
                if (atype.InnerType.MId != Soal.Byte.MId && !arrayNames.Contains(aname))
                {
        ...
        public static Namespace GetNamespace(this SoalType type, Namespace currentNamespace)
        {
            ...
            if (type is ArrayType)
            {
                if (((ArrayType)type).InnerType.MId == Soal.Byte.MId) return SoalGenerator.XsdNamespace;
        ...
        public static string GetXsdName(this SoalType type)
        {
            ...
            if (type is ArrayType)
            {
                if (((ArrayType)type).InnerType.MId == Soal.Byte.MId) return "base64Binary";
