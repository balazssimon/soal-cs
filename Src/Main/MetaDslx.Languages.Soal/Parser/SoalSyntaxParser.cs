    ...
    public class SoalSyntaxParser : Antlr4SyntaxParser<SoalLexer, SoalParser>
    {
        ...
        private class Antlr4ToRoslynVisitor : SoalParserBaseVisitor<GreenNode>
        {
			...
			public override GreenNode VisitOperationHead(SoalParser.OperationHeadContext context)
			{
				...
				return this.factory.OperationHead(annotationList, operationResult, name, tOpenParen, parameterList, tCloseParen, kThrows, throwsList, true);
			}
			...
			{
				if (context == null) return null;
			    SoalParser.ThrowsExceptionContext[] throwsExceptionContext = context.throwsException();
			    ITerminalNode[] tCommaContext = context.TComma();
			    ArrayBuilder<GreenNode> throwsExceptionBuilder = ArrayBuilder<GreenNode>.GetInstance(throwsExceptionContext.Length+tCommaContext.Length);
			    for (int i = 0; i < throwsExceptionContext.Length; i++)
			    {
			        throwsExceptionBuilder.Add((ThrowsExceptionGreen)this.Visit(throwsExceptionContext[i]));
			        if (i < tCommaContext.Length)
			        {
			            throwsExceptionBuilder.Add((InternalSyntaxToken)this.VisitTerminal(tCommaContext[i]));
			        }
			    }
				InternalSeparatedSyntaxNodeList throwsException = InternalSeparatedSyntaxNodeList.Create(throwsExceptionBuilder.ToArrayAndFree());
				return this.factory.ThrowsList(throwsException, true);
			}
			
			public override GreenNode VisitThrowsException(SoalParser.ThrowsExceptionContext context)
			{
				if (context == null) return null;
				SoalParser.QualifierContext qualifierContext = context.qualifier();
				QualifierGreen qualifier = null;
				...
				return this.factory.ThrowsException(qualifier, true);
			}
